...
for Dp = Dpr
 ...
 Om*cos(theta_m),           0, -Om*cos(theta_m),              Oc2,             -Op1,       0,            Dp - Dc,                0,              Od1,               0,                0,       0,               -g31,                0,               0;...
    ...
    re(index) = real(rho_21)/Op;
    im(index) = imag(rho_21)/Op;
% rho_44(index)=(1-x(1)-x(2)-x(3))*epslon/Op;
    index = index + 1;
    
end
figure
plot(Dpr/(2*pi*1e6),re,'r',Dpr/(2*pi*1e6),im,'b--','LineWidth',2);
xlabel('\delta p');
 ylabel('\chi');
axis([-5 5,-inf,inf]);
...
% 
% figure(5)
% plot(Dpr/(2*pi*1e6),im,'r--','LineWidth',2);
% xlabel('\delta p');
%  ylabel('\chi');
%  axis([-5 5,-inf,inf]);
...
% toc
% figure
% plot(Dpr/(2*pi*1e6),rho_44,'r','LineWidth',2);
% xlabel('\delta p');
%  ylabel('\chi');
% axis([-5 5,-inf,inf]);
%  set(gca,'FontSize',20,'Fontname', 'Times New Roman'); 
