...
%     Oc2=Oc1;     
 ...
 Om*cos(theta_m),           0, -Om*cos(theta_m),              Oc2,             -Op1,       0,            Dp - Dc,                0,              Od1,               0,                0,       0,               -g31,                0,               0;...
    ...
    re(cc,index) = real(rho_21)/Op;
    im(cc,index) = imag(rho_21)/Op;   
...
% save im;
figure(1)
mesh(Dpr/(2*pi*1e6),xx/a,re);
set(gca,'FontSize',20,'Fontname', 'Times New Roman');
ylabel('x/a','FontSize',30,'Fontname', 'Times New Roman');
xlabel('\delta p','FontSize',30,'Fontname', 'Times New Roman');
zlabel('Re(\chi)','FontSize',30,'Fontname', 'Times New Roman');
hold on
figure(2)
mesh(Dpr/(2*pi*1e6),xx/a,im);
set(gca,'FontSize',20,'Fontname', 'Times New Roman');
ylabel('x/a','FontSize',30,'Fontname', 'Times New Roman');
xlabel('\delta p','FontSize',30,'Fontname', 'Times New Roman');
zlabel('Im(\chi)','FontSize',30,'Fontname', 'Times New Roman');
hold on
...
R=6;M=10;lamdp=780*1e-9;
 L=1*1e-4;%这个是介质衍射方向长度，自己设定，越长增益越大
...
% end
% for k=1:3
% Ip(k,index)=diffraction_1D_2000_1(R,M,T1,index,k);
% end
% end
% 
% figure(1)
% plot(OC/(2*pi*1e6),Ip(1,:),'r-',OC/(2*pi*1e6),Ip(2,:),'b-',OC/(2*pi*1e6),Ip(3,:),'k-');
% hold on
% 
% 
% %  
