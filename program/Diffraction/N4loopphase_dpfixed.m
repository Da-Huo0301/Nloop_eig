...
%for Dp = Dpr
    ...
    %N(ss)=N0*exp(-(z-z0)^2/h^2);% 光晶格-高斯分布
%     Oc1=Oc0+Oc11*sin(2*pi*z/a);     
%     Oc2=Oc1;     
 ...
 Om*cos(theta_m),           0, -Om*cos(theta_m),              Oc2,             -Op1,       0,            Dp - Dc,                0,              Od1,               0,                0,       0,               -g31,                0,               0;...
    ...
    re(cc) = real(rho_21)/Op;
    im(cc) = imag(rho_21)/Op;   
...
figure
plot(xx/a,re,'r-',xx/a,im,'b-','LineWidth',2)
set(gca,'FontSize',16,'Fontname', 'Times New Roman');
xlabel('x/a','FontSize',16,'Fontname', 'Times New Roman');
ylabel('\chi_p^R','FontSize',16,'Fontname', 'Times New Roman');
grid on
hold on
% figure(2)
% plot(xx/a,im,'b--','LineWidth',3);
% set(gca,'FontSize',20,'Fontname', 'Times New Roman');
% xlabel('x/a','FontSize',30,'Fontname', 'Times New Roman');
% ylabel('\chi_p^I','FontSize',30,'Fontname', 'Times New Roman');
% grid on
% hold on

R=4;M=8;lamdp=780*1e-9;
 L=0.6*1e-5;%这个是介质衍射方向长度，自己设定，越长增益越大

for cc=1:2001
T1(cc)=exp(-2*pi*L*im(cc)/lamdp)*exp(1i*2*pi*L*re(cc)/lamdp);

end

[BIpX1]=diffraction_1D_2000_1(R,M,T1);



sQ=-1.000:0.004:1;
figure
plot(sQ,BIpX1,'k-','LineWidth',2);
set(gca,'FontSize',20,'Fontname', 'Times New Roman');
xlabel('sin(\theta)','FontSize',30,'Fontname', 'Times New Roman');
ylabel('I_p','FontSize',30,'Fontname', 'Times New Roman');
hold on
